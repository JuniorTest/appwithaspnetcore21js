...
namespace SpyStore.Dal.Repos
{
    public class ProductRepo : RepoBase<Product>, IProductRepo
    {
        ...
        public override IEnumerable<Product> GetAll()
            => base.GetAll(x => x.Details.ModelName);
        ...
        public IList<Product> Search(string searchString) 
            => Table.Where(p => EF.Functions.Like(p.Details.Description, $"%{searchString}%") || EF.Functions.Like(p.Details.ModelName, $"%{searchString}%"))
                    .Include(p => p.CategoryNavigation)
                    .OrderBy(x => x.Details.ModelName)
                    .ToList();
